...
use rustc_middle::mir::{BasicBlock, BasicBlockData, Body, LocalDecl, Operand, TerminatorKind};
...
use crate::analysis::type_analysis::{
    self, mir_body, DefaultOwnership, FindPtr, OwnerPropagation, RawGeneric, RawGenericFieldSubst,
    RawGenericPropagation, RawTypeOwner, TypeAnalysis,
};
use crate::components::display::{self, Display};
use crate::context::RcxMut;
...
impl<'tcx, 'a> TypeAnalysis<'tcx, 'a> {
    ...
    //     pt2 Array: [bool;N] indicates whether each generic parameter will have a raw/isolated param
    //
    // Those 2 parts can accelerate heap-ownership inference in the data-flow analysis.
    //
    // Theoretically:
    // AdtDefAnalysis (CollectTypes -> ExtractAdtDefs -> InverseTopSort -> HeapItemUnit -> IsolatedParameter)
    // EncodeField (HeapItemUnit -> IndexedIsolatedParameter)
    pub fn visitor(&mut self) {
        ...
        fn show_owner_if_needed(ref_type_analysis: &mut TypeAnalysis) {
            if !type_analysis::is_display_verbose() {
                return;
            }
        ...
        for each_mir in mir_keys {
            ...
            // Insert the defid to hashset if is not existed and visit the body. Insert ty_map and adt_recorder.
    ...
    //
    // Note that RAP is only interested in 'raw' params ('T' not like '*mut T'). -> *mut T would not be a heap object
                                ...
                                match generic_arg.unpack() {
                                    GenericArgKind::Type(g_ty) => {
                                        let mut raw_generic_field_subst =
                                            RawGenericFieldSubst::new(self.tcx());
