...
impl<'tcx> TyCtxtExtension<'tcx> {
    ...
    // The current version is based on nightly-2023-11-24 -> Update into: nightly-2023-11-23
    pub fn get_def_path(&self, def_id: DefId) -> Vec<Symbol> {
        ...
        impl<'tcx> Printer<'tcx> for AbsolutePathPrinter<'tcx> {
            ...
            // with_no_trimmed_paths! is used to prevent the path from being trimmed. -> `std::vec::Vec` would not be trimmed to `Vec`
            fn path_qualified(
                &mut self,
                self_ty: Ty<'tcx>,
