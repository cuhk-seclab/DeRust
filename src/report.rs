...
// export RUDRA_REPORT_PATH = ""
pub fn default_report_logger(os_string: OsString) -> Box<dyn ReportLogger> {
    Box::new(FileLogger::new(os_string))
...
impl Report {
    ...
    where
        T: Into<Cow<'static, str>>, // From, Into traits
...
impl FileLogger {
    fn new<T>(val: T) -> Self
    where
        T: Into<PathBuf>, // Into: convert T into PathBuf trait
    {
        FileLogger {
            reports: Mutex::new(Vec::new()),
            file_path: val.into(),
...
impl ReportLogger for FileLogger {
    ...
    fn flush(&self) {
        ...
        if !reports.is_empty() {
            ...
            println!("Writing Rudra report to file: {:?}", &self.file_path);
            fs::write(
                &self.file_path,
