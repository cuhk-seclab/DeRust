...
fn show_version() {
    let version = format!("rudra {}", env!("CARGO_PKG_VERSION"));
    println!("The Rudra version: {}", version);
}
...
fn main() {
    ...
    let arg_string = std::env::args().nth(1).unwrap_or_else(|| {
        show_error("`cargo-rudra` must be called with either `rudra` or `rustc` as first argument.")
    });

    // if let Some("rudra") = std::env::args().nth(1).as_ref().map(AsRef::as_ref) {
    if arg_string == String::from("rudra") {
...
fn in_cargo_rudra() {
    ...
    for target in targets {
            ...
            .expect("failed to wait for subprocess")
        {
            Some(exit_status) => {
                if !exit_status.success() {
                    show_error("Finished with non-zero exit code"); // ToDO
                }
            }
            None => {
