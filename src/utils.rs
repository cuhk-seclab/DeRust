...
impl ColorStack {
    ...
    pub fn handle_event(&mut self, event: &ColorEvent) {
        match event.color {
            Some(color) => self.0.push((color, event.id)), // self.0 is the reference to the Vec<(Color, ColorEventId)>
            None => {
...
pub struct ColorSpan<'tcx> {
    ...
    id_counter: usize,
    sub_span_events: Vec<ColorEvent>, // Function span and its statement sub-spans
}
...
impl<'tcx> ColorSpan<'tcx> {
    ...
    pub fn to_colored_string(&self) -> String {
        ...
        if let (Ok((start_loc, end_loc)), Ok(snippet)) = (
            ...
            // Each for loop, handle one line with its column offset.
            for (line_idx, line_content) in (start_line..=end_line).zip(snippet.lines()) {
                ...
                // Closure function: Color the events
                ...
                // Iterate over each column char in the line
...
pub fn replace_experiment_path(path_buf: PathBuf) -> PathBuf {
    let mut new_path = PathBuf::new();

    for component in path_buf.components() {
        new_path.push(component);
        if let Some(component_str) = component.as_os_str().to_str() {
            // if component_str == "experiments" {
            if component_str == "crates" {
                new_path.push("bug_logs");
            }
        }
    }
